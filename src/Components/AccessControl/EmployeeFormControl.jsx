...
export default function EmployeeFormControl(props) {
  ...
  const addEmployee = async (data) => {
    await axios.get(`http://${ipControl}/add/${props.data.end_ip}/21/${data.matrículaTécnico}/${data.senhaTécnico}`);
  };

  const resetEmployee = async (position) => {
    await axios.get(`http://${ipControl}/reset/${props.data.end_ip}/${position}`);
  };
