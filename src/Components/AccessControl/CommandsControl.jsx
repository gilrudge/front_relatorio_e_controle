...
import { validateHour } from '../../../utils/validate';
...
export default function CommandsControl(props) {
  ...
  //Endpoints comandos
  const endPointCommand = `http://${ipControl}/enable-door/${props.data.end_ip}`
  const endPointPort = `http://${ipControl}/open-door/${props.data.end_ip}`
  const endPointHour = `http://${ipControl}/hour-door/${props.data.end_ip}`
  const endPointFacial = `http://${ipControl}/facial-rec/${props.data.end_ip}`
          ...
          <Box style={{ display: 'flex', flexDirection: 'column', gap: '30px', paddingBottom: '30px' }}>
            ...
            <Box style={{ display: 'flex', flexDirection: 'row', gap: '250px' }}>
              <Box sx={{ '& .MuiButton-root': { width: '19.7ch', height: '5ch', mr: 1 } }}>
                <Button onClick={() => { facialRec(1) }} variant="contained">Principal</Button>
                <Button onClick={() => { facialRec(2) }} variant="contained">Acessibilidade</Button>
                <Button disabled onClick={() => { facialRec(3) }} variant="contained">Caixas</Button>
                <Button disabled onClick={() => { facialRec(4) }} variant="contained">Aço</Button>
              </Box>
